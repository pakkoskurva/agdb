...
pub const JITCode = struct {
    ...
    pub fn alloc(allocator: std.mem.Allocator, max_size: usize) !Self {
        const aligned = std.mem.alignForward(usize, max_size, std.mem.page_size);
        if (!comptime is_linux) {
            const mem = try allocator.alignedAlloc(u8, std.mem.page_size, aligned);
            @memset(mem, 0xCC);
...
pub const x86_64_Emitter = struct {
    ...
    pub fn emitNop(self: *Self) !void {
        try _ = self.code.emit(&[_]u8{0x90});
    }
