...
pub const IOUring = struct {
    ring_fd: i32,
    sq_ring: []align(std.mem.page_size) u8,
    cq_ring: []align(std.mem.page_size) u8,
    sqes: []align(std.mem.page_size) io_uring_sqe,
    ...
    pub fn init(entries: u32, flags: u32) IOUringError!Self {
        ...
        const ring_fd = linux_io_uring_setup(entries, &params) catch |err| {
            _ = err;
        ...
        const sqes: []align(std.mem.page_size) io_uring_sqe = blk: {
            const count = sqes_size / @sizeOf(io_uring_sqe);
            const ptr: [*]align(std.mem.page_size) io_uring_sqe = @ptrCast(@alignCast(sqes_raw.ptr));
    ...
    pub fn deinit(self: *Self) void {
        ...
        const sqes_raw: []align(std.mem.page_size) u8 = @as([*]align(std.mem.page_size) u8, @ptrCast(self.sqes.ptr))[0 .. self.params.sq_entries * @sizeOf(io_uring_sqe)];
...
fn mapRing(fd: i32, offset: u64, size: usize) ![]align(std.mem.page_size) u8 {
    if (!comptime is_linux) return error.NotSupported;
...
fn linux_io_uring_setup(entries: u32, params: *io_uring_params) !i32 {
    ...
    const ret = std.os.linux.syscall2(SYS_io_uring_setup, entries, @intFromPtr(params));
