...
pub fn MPSCQueue(comptime T: type) type {
    return struct {
        ...
        head: std.atomic.Value(*Node) align(64),
        tail: std.atomic.Value(*Node) align(64),
        stub: Node,
        allocator: std.mem.Allocator,
        ...
        pub fn init(allocator: std.mem.Allocator) Self {
            ...
            self.stub.next = std.atomic.Value(?*Node).init(null);
            self.head = std.atomic.Value(*Node).init(&self.stub);
            self.tail = std.atomic.Value(*Node).init(&self.stub);
            return self;
        }
        ...
        pub fn dequeue(self: *Self) ?T {
            ...
            if (tail == &self.stub) {
                ...
                self.tail.store(n, .release);
                const next2 = n.next.load(.acquire);
                if (next2) |nx| {
                    self.tail.store(nx, .release);
                    const value = n.value;
        ...
        pub fn isEmpty(self: *const Self) bool {
            const tail = self.tail.load(.acquire);
            return tail.next.load(.acquire) == null;
        }
    };
}
